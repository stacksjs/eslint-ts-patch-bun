/**
 * This file adds a Node CJS register hook to kinda hijack ESLint's internal module
...
const debug = require('debug')('eslint-ts-patch-bun')
...
const replacers = [
  {
    ...
    replace: content => content
       ...
       *
       * We use Bun to load typescript files, and use native `import` to load javascript files.
       * An extra `.default` interop.
       */
      .replace(
        'const config = (await import(fileURL)).default;',
        [
          'let config = await import(fileURL);',
...
extensions['.js'] = function (module, filename) {
  ...
  if (replacer) {
    ...
    if (replaced === content)
      throw new Error(`[eslint-ts-patch-bun] ${replacer.path} is not patched! It might because ESLint changes the internal code structure. Please report this to https://github.com/stacksjs/eslint-ts-patch-bun`)
